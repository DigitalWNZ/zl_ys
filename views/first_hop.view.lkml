view: first_Hop {
  derived_table: {
    sql: select
      ...
      where diagtype=2
      and (if(array_length(Tracert)=0,null,tracert[offset(0)].Hop) is not null and if(array_length(Tracert)=0,null,tracert[offset(0)].Hop) != '@' and if(array_length(Tracert)=0,null,tracert[offset(0)].Hop) != '*')
      and (if(array_length(Tracert)=0,null,tracert[offset(0)].Delay) is not null and if(array_length(Tracert)=0,null,tracert[offset(0)].Delay) != '@' and if(array_length(Tracert)=0,null,tracert[offset(0)].Delay) != '*')
       ;;
  }
