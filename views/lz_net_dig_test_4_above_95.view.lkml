view: lz_net_dig_test_4_above_95 {
  derived_table: {
    sql: with raw_p95 as (SELECT
      ...
      (FORMAT_TIMESTAMP('%F %T', TIMESTAMP_SECONDS(30*60 * DIV(UNIX_SECONDS(timestamp((FORMAT_TIMESTAMP('%F %T', lz_net_dig_test.EventTime)))), 30*60)) )) AS lz_net_dig_test_timekey_30_time,
      ...
      percentile_cont(average,0.95) over(partition by geo_ip_country_range.country_iso_code,(FORMAT_TIMESTAMP('%F %T', TIMESTAMP_SECONDS(30*60 * DIV(UNIX_SECONDS(timestamp((FORMAT_TIMESTAMP('%F %T', lz_net_dig_test.EventTime)))), 30*60))))) as p95,
      row_number() over(partition by geo_ip_country_range.country_iso_code,(FORMAT_TIMESTAMP('%F %T', TIMESTAMP_SECONDS(30*60 * DIV(UNIX_SECONDS(timestamp((FORMAT_TIMESTAMP('%F %T', lz_net_dig_test.EventTime)))), 30*60))))) as seq
